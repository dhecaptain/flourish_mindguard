...
export async function GET() { const user = await getUser(); if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 }); const [profile] = await db.select().from(wellnessProfile).where(eq(wellnessProfile.userId, user.id)).limit(1); return NextResponse.json(profile ?? { userId: user.id, displayName: user.name, streak: 0, notificationsEnabled: true, gentleModeEnabled: false }) }
export async function PATCH(request: Request) { const user = await getUser(); if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 }); const body = await request.json(); const values = { displayName: typeof body.displayName === 'string' ? body.displayName.trim().slice(0, 80) || user.name : user.name, notificationsEnabled: Boolean(body.notificationsEnabled), gentleModeEnabled: Boolean(body.gentleModeEnabled), updatedAt: new Date() }; await db.insert(wellnessProfile).values({ userId: user.id, ...values }).onConflictDoUpdate({ target: wellnessProfile.userId, set: values }); return NextResponse.json(values) }
